...
// import RawMaterialsConsumptionModal from '#components/RawMaterialsConsumption/RawMaterialsConsumptionModal.jsx';
...
//   const [activeBatchId, setNextBatchId] = useState(0);
...
//       });
//     console.log('filtered CackeFillUp.jsx line 113', filtered);
...
function CackeFillUp() {
  ...
  const handleProductionPlanRowClick = (row) => {
    const prodPlanEntry = batchOutside.find(
      (el) => el.id === row.original.id || el.id == cackeFillUp.id,
    );
    if (!prodPlanEntry) return;
